...
test_data.loc[(test_data['Hour_of_day'] >= 7) & (test_data['Hour_of_day'] <= 9), 'is_rush_hour'] = 1  
test_data.loc[(test_data['Hour_of_day'] >= 17) & (test_data['Hour_of_day'] <= 19), 'is_rush_hour'] = 1
...
#seperating data for validation
...
train_generator = TimeseriesGenerator(train_X, train_Y, length = timesteps, sampling_rate=1, batch_size=24)
test_generator = TimeseriesGenerator(test_X, test_Y, length=timesteps, sampling_rate=1, batch_size=24)
...
#model.summary()
#plot_model(model, to_file='model_plot_noCOVID.png', show_shapes=True, show_layer_names=True)
